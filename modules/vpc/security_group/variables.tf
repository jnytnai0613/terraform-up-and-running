...
variable "security_group_ingress_ports" {
  type        = list(number)
  description = "内向き通信として許可するポート番号のリスト"
  default = [
    80
  ]
}
...
variable "security_group_ingress_cidrs" {
  type        = list(string)
  description = "内向き通信として許可するCIDのリスト"
  default     = []
}

variable "security_group_ingress_sgs" {
  type        = list(string)
  description = "内向き通信との送信元セキュリティグループとして許可するセキュリティグループIDのリスト"
  default     = []
}

variable "security_group_ingress_allow_self" {
  type        = bool
  description = "内向き通信として自身のセキュリティグループを許可するかどうか"
  default     = false
}
