...
variable "alb_log_expiration_in_days" {
  ...
  validation {
    condition     = var.alb_log_expiration_in_days > 365
    error_message = "alb_log_expiration_in_days must be greater than 1 year (365 days)."
  }
}
...
locals {
  ...
  # ログ格納バケットの設定
  alb_log_bucket_name          = "${local.alb_name}-log-bucket-taniai"
