...
variable "task_role_managed_policy_arns" {
  description = "タスクロールにアタッチするマネージドポリシーのARN一覧"
  type        = list(string)
  default     = []

  validation {
    condition     = length(var.task_role_managed_policy_arns) < 10
    error_message = "Number of managed policies attached to the task role must be less than 10."
  }
}

variable "task_role_inline_policies" {
  description = "タスクロールにアタッチするインラインポリシーのマップ"
  type        = map(string)
  default     = {}
}
